...
def test_create_depths_plot_pv(electrode_names, electrodes, raw):
    mne.viz.set_3d_backend('pyvista')
    depth_list = seeg.create_depths(electrode_names, raw.info['ch_names'],
                                    electrodes)
    brain = seeg.create_depths_plot(depth_list, SUBJECT_ID, SUBJECTS_DIR)
...
def test_show_bipolar_values_pv(electrode_names, electrodes, raw, T_x_inv):
    mne.viz.set_3d_backend('pyvista')
    depth_list = seeg.create_depths(electrode_names, raw.info['ch_names'],
                                    electrodes)
    brain = seeg.create_depths_plot(depth_list, SUBJECT_ID, SUBJECTS_DIR)
...
def test_create_depth_source_image_map_mlab(eeg, freqs, electrode_names,
...
def test_create_depth_source_image_map_pv(eeg, freqs, electrode_names,
                                          electrodes, raw, T_x_inv):
    mne.viz.set_3d_backend('pyvista')
    values = seeg.create_depth_source_image_map(eeg, freqs,
                                                low_freq=120, high_freq=200)
